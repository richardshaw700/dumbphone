# Bigme Hibreak Pro - Apps

- [Phone](#phone)
- [Unencrypted](#unencrypted-chat)
- [Semi-encrypted](#semi-encrypted-chat)
- [Encrypted](#encrypted-chat)
- [AI Chat](#ai-chat)
- [Wallet](#wallet)
- [Unencrypted Maps](#unencrypted-maps)
- [Encrypted Maps](#encrypted-maps)
- [Settings](#settings)
- [Weather](#weather)
- [Calculator](#calculator)
- [Camera](#camera)
- [Photos](#photos)
- [Email](#email)
- [Doc Scanner](#doc-scanner)
- [AI Note Taker](#ai-note-taker)
- [TD Bank](#td-bank)
- [Scotiabank](#scotiabank)
- [Smart Devices](#smart-devices)
- [Contacts](#contacts)
- [Thermostat](#thermostat)
- [Quo](#quo)

---

## Phone

**Stock** - Built-in dialer

## Unencrypted Chat

**Google Messages** (Stock) - SMS/MMS, RCS when available

## Semi-encrypted Chat

**WhatsApp** - E2EE for content, but Meta collects metadata
[Play Store](https://play.google.com/store/apps/details?id=com.whatsapp)

## Encrypted Chat

**Signal** - Full E2EE, minimal metadata collection
[Play Store](https://play.google.com/store/apps/details?id=org.thoughtcrime.securesms)

## Wallet

**Google Wallet** - Tap-to-pay, cards, passes
[Play Store](https://play.google.com/store/apps/details?id=com.google.android.apps.walletnfcrel)

## Unencrypted Maps

**Google Maps** (Stock) - Navigation, directions, traffic, Street View

## Encrypted Maps

**Organic Maps** - Offline, privacy-first navigation with OpenStreetMap
[Play Store](https://play.google.com/store/apps/details?id=app.organicmaps)

## Settings

**Stock** - System settings

## Weather

**SimpleWeather** - Free, ad-free weather app
[Play Store](https://play.google.com/store/apps/details?id=com.thewizrd.simpleweather)

## Calculator

**Stock** - Built-in calculator

## Camera

**Stock** - Built-in camera

## Photos

**Fossify Gallery** - Private, no ads, no tracking
[Play Store](https://play.google.com/store/apps/details?id=org.fossify.gallery)

## Email

**FairEmail** - Privacy-focused, direct IMAP connection (no middleman)
[F-Droid](https://f-droid.org/en/packages/eu.faircode.email/)

## Doc Scanner

**FairScan** - Simple, offline PDF scanner with auto-detect and crop
[F-Droid](https://f-droid.org/en/packages/org.fairscan.app/)

## AI Note Taker

**Otter.ai** - Meeting transcription, 300 free mins/month
[Play Store](https://play.google.com/store/apps/details?id=com.aisense.otter)

## AI Chat

**ChatGPT** - OpenAI assistant
[Play Store](https://play.google.com/store/apps/details?id=com.openai.chatgpt)

## TD Bank

**TD Canada** - Banking app
[Play Store](https://play.google.com/store/apps/details?id=com.td)

## Scotiabank

**Scotiabank** - Banking app
[Play Store](https://play.google.com/store/apps/details?id=com.scotiabank.banking)

## Smart Devices

**Kasa** - TP-Link smart home control
[Play Store](https://play.google.com/store/apps/details?id=com.tplink.kasa_android)

## Contacts

**Fossify Contacts** - Local only, no cloud sync, open source
[F-Droid](https://f-droid.org/en/packages/org.fossify.contacts/)

## Thermostat

**Ecobee** - Smart thermostat control
[Play Store](https://play.google.com/store/apps/details?id=com.ecobee.athenamobile)

## Quo

**Quo** - Customer service / business phone
[Play Store](https://play.google.com/store/apps/details?id=com.openphone)
